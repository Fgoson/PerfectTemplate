...
public final class MySQLStmt {
	...
	public func execute() -> Bool {
		if paramBindsOffset > 0 {
			guard let paramBinds = self.paramBinds,
				false == mysql_stmt_bind_param(ptr, paramBinds) else {
					return false
			}
		}
	...
	public final class Results: IteratorProtocol {
		...
		let isNullBuffers: UnsafeMutablePointer<Bool>
		...
		init(_ s: MySQLStmt) {
			...
			isNullBuffers = UnsafeMutablePointer<Bool>.allocate(capacity: numFields)
