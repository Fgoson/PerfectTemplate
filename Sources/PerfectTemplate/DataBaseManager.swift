...
//  PerfectTemplate
//
//  Created by honpe on 2018/12/13.
//
...
open class DataBaseManager {
    ...
    //////////
    func selectDataBaseSQLwhere(tableName: String, keyValue: String) -> [Dictionary<String, String>]? {
//        selectAllDataBaseSQLwhere(tableName: table_account, keyValue: "level_id=1001")
        let SQL = "SELECT * FROM \(tableName) WHERE \(keyValue)"
        return mysqlGetDataResult(SQL)
    }
    //公共方法获取account_level表中数据返回
    func mysqlGetDataResult(_ sql: String) -> [Dictionary<String, String>]? {
        let result = mysqlStatement(sql)
        var resultArray = [Dictionary<String, String>]()
        var dic = [String:String]()
        result.mysqlResult?.forEachRow(callback: { (row) in
            dic["accountLevelId"] = row[0]
